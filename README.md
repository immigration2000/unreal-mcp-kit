...
## UE 5.7 (공식 MCP 없음 → 커뮤니티 서버)

UE 5.7에는 Epic 공식 `ModelContextProtocol`이 **없습니다**(5.8부터). 그래서 위 `setup_project.py`는 5.7에서 동작하지 않고, **커뮤니티 서버**를 씁니다. 상세 절차는 **`setup-ue57` 스킬**에 있습니다.

- **VibeUE** — 블루프린트/에셋/에디터 툴, HTTP MCP `http://127.0.0.1:8088/mcp` (Fab 또는 `git clone` + `buildplugin.bat`).
- **UnrealClaude**(선택) — 뷰포트 캡쳐 + 액터 조작, MCP 포트 3000 (소스 빌드 + `mcp-bridge`에서 `npm install`).
- Claude Code 연결: `claude mcp add --scope user --transport stdio VibeUE-Claude -- npx -y mcp-remote http://127.0.0.1:8088/mcp --transport http-only --allow-http`
- 프로젝트 지침: 루트 `CLAUDE.md`에 `@Plugins/VibeUE/Content/samples/AGENTS.md.sample` import.
- 엔진 gotcha 지식(`ue5-8-mcp`)은 server-agnostic이라 **5.7에도 그대로 적용**됩니다.

> 5.7 서버는 **에디터가 열려 있을 때만** 동작합니다. 자세한 설치·검증·문제해결은 `skills/setup-ue57/SKILL.md` 참고.
